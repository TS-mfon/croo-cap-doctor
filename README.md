...
Independent CROO production agent. Price: **1.50 USDC**.
...
npm run check
npm run dev
curl -X POST http://localhost:3000/v1/analyze -H 'content-type: application/json' -d '{"target_service_id":"svc","mode":"schema","max_price_microusdc":100000,"repetitions":1}'
```
